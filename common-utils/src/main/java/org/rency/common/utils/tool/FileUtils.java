 ...
 * <p>文件处理工具类</p>
 * @author sean won
 * @version $Id: FileUtil.java, v 0.1 2009-10-7 下午03:08:53 fuyangbiao Exp $
 */
public class FileUtils {
    ...
    public static String getFileExt(String fileName) {
        ...
        int dotPos = fileName.lastIndexOf(EXTENSION_SEPARATOR);
        if (fileName.lastIndexOf(EXTENSION_SEPARATOR) >= 0) {
    ...
    public static String getFileNameNoExt(String fileName) {
        ...
        int dotPos = fileName.lastIndexOf(EXTENSION_SEPARATOR);
        if (dotPos >= 0) {
            noext = fileName.substring(0, dotPos);
        } else {
            noext = fileName;
        }
    ...
    public static String getFileName(String filePath) {
        ...
        int dotPos = filePath.lastIndexOf(SLASH);
        if (dotPos >= 0) {
            fileName = filePath.substring(dotPos + 1, filePath.length());
        } else {
            fileName = filePath;
        }
    ...
    public static boolean move(String sourceFile, String destDirection) {
        ...
        File newFile = new File(destDirection + SLASH + getFileName(sourceFile));
        // 移动文件
